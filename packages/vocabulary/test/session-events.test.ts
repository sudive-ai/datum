...
test('the core vocabulary holds exactly the 12 named persistent event types', () => {
  assert.equal(KNOWN_SESSION_EVENT_TYPES.length, 12)
  assert.equal(new Set(KNOWN_SESSION_EVENT_TYPES).size, 12)
